...
UNSTRESSED_FUNCTION_WORDS = {
    "a", "an", "and", "as", "at", "be", "but", "by", "for", "from", "if", "in", "into",
    "is", "it", "nor", "of", "on", "or", "so", "than", "that", "the", "their", "them",
    "then", "there", "these", "they", "this", "to", "up", "was", "we", "were", "what",
    "when", "where", "which", "who", "with", "you", "your",
}

STRESSED_SUFFIXES = (
    "tion", "sion", "cian", "ture", "gion", "self", "selves", "ique", "eer",
)

WEAK_SUFFIXES = (
    "ing", "ed", "es", "ly", "er", "est", "ous", "less", "ness",
)
...
def estimate_syllables(word: str) -> int:
    ...
    # Subtract silent trailing e where it is likely silent.
    if clean.endswith("e") and not clean.endswith(("le", "ye")) and syllables > 1:
        syllables -= 1

    # Ensure at least one syllable for alphabetic words.
...
def estimate_stress_pattern(word: str) -> str:
    ...
    # Default heuristic: lexical words often stress near the front in English.
